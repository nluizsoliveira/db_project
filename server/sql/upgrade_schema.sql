...
CREATE TABLE USUARIO_SENHA (
    -- Campos conforme especificação do PF
    USERID VARCHAR(11) NOT NULL,  -- UserID (usando CPF como identificador único)
    LOGIN VARCHAR(255) NOT NULL,   -- Login (email do usuário)
    SENHA VARCHAR(255) NOT NULL,   -- Senha (hash MD5)
    TIPO VARCHAR(50),              -- Tipo de usuário (Administrador, Staff, Interno, Externo)
    IDORIGINAL VARCHAR(11) NOT NULL, -- IdOriginal (CPF da pessoa na tabela de origem)

    -- Campos adicionais para funcionalidades do sistema
    SENHA_HASH VARCHAR(255) NOT NULL, -- Mantido (mesmo valor que SENHA) para compatibilidade
    ...
    CONSTRAINT PK_USUARIO_SENHA PRIMARY KEY (USERID),
    CONSTRAINT FK_USUARIO_SENHA_PESSOA FOREIGN KEY (USERID)
        REFERENCES PESSOA (CPF)
            ON DELETE CASCADE
            ON UPDATE CASCADE,
    CONSTRAINT UN_USUARIO_SENHA_LOGIN UNIQUE (LOGIN),
    CONSTRAINT CK_USUARIO_SENHA_TIPO CHECK (
        TIPO IN ('Administrador', 'Staff', 'Interno', 'Externo') OR TIPO IS NULL
    ),
    CONSTRAINT CK_USUARIO_SENHA_SENHA CHECK (SENHA = SENHA_HASH),
    CONSTRAINT CK_USUARIO_SENHA_IDORIGINAL CHECK (IDORIGINAL = USERID)
);

-- 23.
CREATE TABLE AUDITORIA_LOGIN (
    -- Campos conforme especificação do PF (log_table)
    USERID VARCHAR(11),           -- UserID (pode ser NULL para tentativas de login falhadas)
    DATA_HORA_LOGIN TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- Data e hora do login

    -- Campos adicionais para funcionalidades do sistema
    ID_LOG INT GENERATED BY DEFAULT AS IDENTITY,
    TIMESTAMP_EVENTO TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- Mantido (mesmo valor que DATA_HORA_LOGIN)
    EMAIL_USUARIO VARCHAR(255),    -- Mantido para auditoria mesmo quando UserID não está disponível
    IP_ORIGEM VARCHAR(45),
    STATUS VARCHAR(20) NOT NULL,
    MENSAGEM TEXT,

    CONSTRAINT PK_AUDITORIA_LOGIN PRIMARY KEY (ID_LOG),
    CONSTRAINT FK_AUDITORIA_LOGIN_USER FOREIGN KEY (USERID)
        REFERENCES USUARIO_SENHA (USERID)
            ON DELETE SET NULL
            ON UPDATE CASCADE,
