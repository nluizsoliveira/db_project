...
const IconMenuItem = ({ href, label, description, icon: Icon, isActive }: IconMenuItemProps) => (
  <li>
    <NavigationMenuLink
      asChild
    ...
    >
      <Link href={href} className="flex items-start gap-3 rounded-md px-3 py-2 hover:no-underline">
        <div className="mt-0.5 rounded-md bg-muted p-2 text-muted-foreground">
          <Icon className="size-4" />
        </div>
        <div className="space-y-0.5">
          <p className="text-sm font-medium leading-none">{label}</p>
          <p className={`text-xs leading-snug ${isActive ? 'text-white/90' : 'text-muted-foreground'}`}>
            {description}
          </p>
        </div>
      </Link>
...
const Navbar = memo(function Navbar() {
  ...
  const renderMenuGroup = (label: string, Icon: LucideIcon, items: NavItem[]) => (
    <NavigationMenuItem>
      <NavigationMenuTrigger
        data-active={items.some((item) => isActiveRoute(item.href)) ? 'true' : 'false'}
        className="gap-2"
      >
        <Icon className="size-4" />
        {label}
      ...
      <NavigationMenuContent className="!left-0 !right-auto z-50 md:z-[70] max-w-[calc(100vw-3rem)]">
        <ul className="grid w-[320px] gap-2 sm:w-[420px] md:w-[520px] md:grid-cols-2">
          {items.map((item) => (
            <IconMenuItem key={item.href} {...item} isActive={isActiveRoute(item.href)} />
          ))}
        </ul>
      </NavigationMenuContent>
    </NavigationMenuItem>
            ...
            <NavigationMenuList className="flex-wrap">
              {visibility.showReports && (
                <NavigationMenuItem className="self-center">
                  ...
                  >
                    <Link href="/reports/overview" className="flex items-center gap-2">
              ...
              {visibility.showAdmin && renderMenuGroup('Administração', ShieldCheck, adminItems)}
              {visibility.showStaff && renderMenuGroup('Área do Funcionário', UserCog, staffItems)}
              {visibility.showInternal && renderMenuGroup('Área USP', University, internalItems)}
              {visibility.showExternal && (
                <NavigationMenuItem>
                  ...
                  >
                    <Link href="/external/dashboard" className="flex items-center gap-2">
